...
# # 配置数据库连接
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql+pymysql://root:123456@127.0.0.1:3306/leava'
        ...
        if not (request.json):
            ...
            if len(str(rec_id)) == 6:
                ...
                if (db_data) is None:
                    ...
                    db_id = str(Decimal(db_data['teacher_id']).quantize(Decimal('0')))
            ...
            if len(str(rec_id)) == 8:
                ...
                if (db_data) is None:
                    ...
                    db_id = str(Decimal(db_data['student_id']).quantize(Decimal('0')))
                    db_name = db_data['name']
                    db_pwd = db_data['passwd']
                    ...
                    if check_password_hash(str(db_pwd),str(rec_pwd)) is not True:
                        ...
                        return_data['name'] = db_name
                        return_data['s_class'] = db_class
                        return_data['room'] = db_room
                        return_data['flag'] = '2'
                        return_data['school']=db_school
                        return (jsonify(return_data))
...
def insert_leave(student_id, start_time, end_time, reason, flag, teacher1_id, teacher2_id, s_type, ensure):
    engine = create_engine(
        'mysql+pymysql://root:123456@127.0.0.1:3306/leava')
...
def search_t_id(school):
    ...
    if result is None:
        ...
        while i < result_length:
            ...
            teacher_id = str(Decimal(result[i].to_dict()['teacher_id']).quantize(Decimal('0')))
            name = result[i].to_dict()['name']
